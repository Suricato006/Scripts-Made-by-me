...
elseif (game.PlaceId == 6461766546) then
    local Window = Library:CreateWindow("A Hero Destiny")
    Window.open = false
    ...
    for i, v in pairs(game:GetService("Workspace").Spawns:GetChildren()) do
        if not table.find(NpcList, v.Name) then
            NpcList[#NpcList + 1] = v.Name
        end
    end
    Window:AddList({text = "Choose Npcs", values = NpcList, value = NpcList[1], callback = function(chosen)
        _G.NpcNameAHD = chosen
    end})

    Window:AddList({text = "Choosen Npcs", values = NpcList, value = NpcList[1], callback = function(chosen)
        _G.NpcNameAHD = chosen
    end})
